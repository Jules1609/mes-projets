...
def _set_geometry_of_domain(M, N, level=0, motif='koch'):
    # -- define geometry
    ...
    # -- define fractal
    nodes = create_fractal_nodes([numpy.array([[0], [N]]), numpy.array([[N], [N]])], level, motif)
...
def create_motif(A, B, motif='koch'):
    """
    ...
    if motif == 'koch':
        alpha = numpy.pi / 2 
    #motif de koch penché :
    elif motif == 'koch tilted':
        alpha = 4 * numpy.pi / 7 
    elif motif == 'flake 1':
        alpha = numpy.pi / 3
    elif motif == 'flake 2':
        ...
        raise ValueError("Unknown motif type: {}".format(motif))
    ...
    if motif == 'koch' or motif == 'koch tilted':
        return [A, C, D, E, F, G, H, I, B]
    elif motif == 'flake 1':
        return [A, C, D, F, H, I, B]
    elif motif == 'flake 2':
        return [A, D, F, H, B]
    elif motif == 'snowflake':
        radius = numpy.array([[numpy.cos(alpha), -numpy.sin(alpha)],
        ...
        C = A + distance 
        D = C + numpy.dot(radius, distance)
        E = D + distance - numpy.dot(radius, distance)
        F = E + distance
        return [A, C, D, E, F, B]
...
def create_fractal_nodes(nodes, n_iter, motif='koch'):
    """
...
def create_fractal_coordinates_prof(nodes, domain):
    """
...
def create_fractal_coordinates(nodes, domain):
    ...
    ..warning: nodes must be precomputed with function create_fractal_nodes
    """
    print("Creating fractal coordinates...")
    ...
    # build integer node coordinates
    node_int = []
    for k in range(len(nodes)):
        xi = int(round(nodes[k][0][0]))
        yi = int(round(nodes[k][1][0]))
        node_int.append((xi, yi))

    # walk each segment with Bresenham and concatenate (avoid duplicate join point)
    coords = []
    for k in range(len(node_int) - 1):
        x0, y0 = node_int[k]
        x1, y1 = node_int[k + 1]
        seg = bresenham(x0, y0, x1, y1)
        if coords and seg and seg[0] == coords[-1]:
            seg = seg[1:]
        coords.extend(seg)

    # clamp to domain and remove consecutive duplicates
    clamped = []
    for xi, yi in coords:
        xi_c = min(max(0, xi), N - 1)
        yi_c = min(max(0, yi), M - 1)
        if not clamped or clamped[-1] != (xi_c, yi_c):
            clamped.append((xi_c, yi_c))

    x = [c[0] for c in clamped]
    y = [c[1] for c in clamped]
